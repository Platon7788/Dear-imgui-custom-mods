...
use dear_imgui_custom_mod::borderless_window::{
    BorderlessConfig, ButtonConfig, CloseMode, ExtraButton, TitleAlign, TitlebarState,
    WindowAction, actions::ResizeEdge, render_titlebar,
};
...
impl DemoState {
    ...
    fn build_cfg(
        ...
        if !separator {
            cfg = cfg.without_separator();
        }
        if !drag_hint {
            cfg = cfg.without_drag_hint();
        }
        cfg
    }
        ...
        ui.window("##root")
            ...
            .build(|| {
                ...
                if self.show_confirm {
                    let dlg_cfg =
                        DialogConfig::new("Close Application", "Are you sure you want to close?")
    ...
    fn render_config_panel(&mut self, ui: &Ui) -> Option<ActiveTheme> {
        ...
        if ui.slider("##h", 20.0_f32, 50.0, &mut self.edit_h) {
            self.rebuild_cfg();
        }
        ...
        ui.set_next_item_width(-1.0);
        if ui.slider("##rz", 3.0_f32, 16.0, &mut self.edit_rz) {
            self.rebuild_cfg();
        }

        // Alignment
        if ui.checkbox("Center Title", &mut self.edit_center) {
            self.rebuild_cfg();
        }

        // Confirm close
        if ui.checkbox("Confirm on Close", &mut self.edit_confirm) {
            self.rebuild_cfg();
        }
        if ui.checkbox("Separator Line", &mut self.edit_separator) {
            self.rebuild_cfg();
        }
        if ui.checkbox("Drag Hint", &mut self.edit_drag_hint) {
            self.rebuild_cfg();
        }
        ...
        if ui.checkbox("Minimize", &mut sm) {
            self.cfg.buttons.show_minimize = sm;
        }
        if ui.checkbox("Maximize", &mut sx) {
            self.cfg.buttons.show_maximize = sx;
        }
        ...
        ui.separator();
        ui.text("State");
        ui.text_disabled(if self.state.maximized {
            "Maximized"
        } else {
            "Normal"
        });
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                if gpu.demo.theme != gpu.last_theme {
                    ...
                    if let Some(hwnd) = hwnd_of(&gpu.window) {
                        dear_imgui_custom_mod::borderless_window::platform::set_titlebar_dark_mode(
                            hwnd,
                            gpu.demo.theme != ActiveTheme::Light,
                        );
                    }
                ...
                let clear = match gpu.demo.theme {
                    ActiveTheme::Light => wgpu::Color {
                        r: 0.89,
                        g: 0.89,
                        b: 0.93,
                        a: 1.0,
                    },
                    ActiveTheme::Solarized => wgpu::Color {
                        r: 0.03,
                        g: 0.19,
                        b: 0.23,
                        a: 1.0,
                    },
                    ActiveTheme::Monokai => wgpu::Color {
                        r: 0.12,
                        g: 0.12,
                        b: 0.12,
                        a: 1.0,
                ...
                match action {
                    ...
                    WindowAction::Maximize => {
                        ...
                        gpu.demo.log.push(format!(
                            "Maximize → {}",
                            if new { "maximized" } else { "restored" }
                        ));
                    }
                    ...
                    WindowAction::ResizeStart(e) => {
                        gpu.window.drag_resize_window(to_winit_resize(e)).ok();
                    }
                    _ => {}
                }
