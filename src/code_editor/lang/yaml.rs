...
fn tokenize(line: &str) -> Vec<Token> {
    ...
    // Directive (%YAML, %TAG)
    if i < len && bytes[i] == b'%' {
        tokens.push(Token { kind: TokenKind::Attribute, start: i, len: len - i });
        return tokens;
    }
    ...
    // Document markers (--- or ...)
    {
        ...
        if trimmed == "---" || trimmed == "..." {
            tokens.push(Token { kind: TokenKind::Keyword, start: i, len: len - i });
            return tokens;
        }
    }
    ...
    while i < len {
        ...
        if b == b'!' {
            ...
            while i < len && bytes[i] != b' ' && bytes[i] != b'\t'
                && bytes[i] != b'\n' && bytes[i] != b',' { i += 1; }
            tokens.push(Token { kind: TokenKind::TypeName, start, len: i - start });
            continue;
        }
        ...
        // ── Unquoted string / bare value ─────────────────────────────────
        {
            ...
            if end > start {
                ...
                tokens.push(Token { kind, start, len: end - start });
            }
            if end < i {
                tokens.push(Token { kind: TokenKind::Whitespace, start: end, len: i - end });
            }
        }
    }
...
mod tests {
    ...
    fn key_value() {
        ...
        assert!(toks.iter().any(|t| t.0 == TokenKind::Identifier && t.1 == "name"));
        assert!(toks.iter().any(|t| t.0 == TokenKind::Operator && t.1 == ":"));
    ...
    fn anchor_alias() {
        ...
        let macros: Vec<_> = toks.iter().filter(|t| t.0 == TokenKind::MacroCall).collect();
    ...
    fn tag() {
        ...
        assert!(toks.iter().any(|t| t.0 == TokenKind::TypeName && t.1 == "!!timestamp"));
    }
