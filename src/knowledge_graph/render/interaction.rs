...
pub(crate) fn handle_drag(
        ...
        && let Some(id) = *dragging_node
    {
        *dragging_node = None;
        if let Some(node) = graph.nodes.get(id) {
            events.push(GraphEvent::NodeMoved(id, node.pos));
        }
    }
...
pub(crate) fn handle_box_select(
    ...
    // Start box select: LMB pressed on empty space.
    if ui.is_mouse_clicked(MouseButton::Left) && hovered_node.is_none() {
...
pub(crate) fn handle_context_menu(
    ...
    ui.popup("##kg_ctx", || {
        ...
        // Pin / Unpin.
        let pinned = graph.node(id).map_or(false, |s| s.pinned);
        let pin_label = if pinned { "Unpin" } else { "Pin" };
        if ui.menu_item(pin_label) {
            if let Some(node) = graph.node_mut(id) {
                node.pinned = !node.pinned;
                events.push(GraphEvent::NodePinned(id, node.pinned));
            }
            sim.wake();
        }
