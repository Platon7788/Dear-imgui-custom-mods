...
pub(crate) fn render(
    ...
    // 10. Pan when dragging on empty area (no node hovered, not box-selecting).
    let lmb_down = canvas_hovered && ui.is_mouse_down(MouseButton::Left);
    if lmb_down && !dragging && ctx.box_select_start.is_none() && hovered_node.is_none() {
    ...
    // 17. Build hover-neighbor set for fade effect.
    let hover_neighbors: Option<HashSet<NodeId>> = if let Some(hov) = *ctx.hovered {
    ...
    for (_, edge) in graph.edges.iter() {
        ...
        let alpha = if let Some(ref nbrs) = hover_neighbors {
            if nbrs.contains(&edge.from) && nbrs.contains(&edge.to) {
                1.0
            } else {
        ...
        let thickness =
            (1.0 + edge.weight * 2.0) * ctx.camera.zoom.clamp(0.5, 2.0) * config.edge_thickness_multiplier;
    ...
    for (node_id, node) in graph.nodes.iter() {
        ...
        let node_alpha = if let Some(ref nbrs) = hover_neighbors {
            if nbrs.contains(&node_id) { 1.0 } else { config.hover_fade_opacity }
        } else {
            1.0
        };
        ...
        if ctx.selection.contains(&node_id) {
            draw.add_circle(screen_pos, screen_radius + 3.0, col(with_alpha(colors.node_selected, node_alpha)))
                .thickness(1.5)
                .num_segments(num_segments)
                .build();
        }
...
fn graph_bounds(graph: &GraphData) -> Option<[[f32; 2]; 2]> {
