...
const POPUP_INPUT_WIDTH: f32 = 240.0;
    ...
    pub(super) fn render_goto_popup(&mut self, ui: &dear_imgui_rs::Ui) {
        ...
        themed_popup_style(ui, || {
            if let Some(_popup) = ui.begin_popup(&self.goto_popup_id) {
                ...
                let (go_clicked, cancel_clicked) = render_action_row(ui, POPUP_INPUT_WIDTH, "Go");
                if cancel_clicked {
    ...
    pub(super) fn render_settings_popup(&mut self, ui: &dear_imgui_rs::Ui) {
        ...
        themed_popup_style(ui, || {
            if let Some(_popup) = ui.begin_popup(&self.settings_popup_id) {
                // Settings hosts ~11 checkboxes + 7 size buttons —
                // the standard `themed_popup_style` (item-spacing
                // `[10, 8]`, frame-padding `[10, 6]`) makes it
                // tower-tall. Override locally with a compact
                // ruleset: tighter rows, square-ish buttons. Pops
                // when the body closure ends (no leakage to
                // sibling popups).
