...
pub(super) fn point_to_segment_dist(p: [f32; 2], a: [f32; 2], b: [f32; 2]) -> f32 {
    ...
    if len_sq < 1e-6 {
        return f32::hypot(p[0] - a[0], p[1] - a[1]);
    }
    let t = (((p[0] - a[0]) * dx + (p[1] - a[1]) * dy) / len_sq).clamp(0.0, 1.0);
    let proj_x = a[0] + t * dx;
    let proj_y = a[1] + t * dy;
    f32::hypot(p[0] - proj_x, p[1] - proj_y)
}
...
// 500-line cap (CLAUDE.md). `tests` is still a child of `math`, so its
// `use super::*` reaches the `pub(super)` helpers directly.
